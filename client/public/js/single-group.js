...
const socket = io('localhost:3000',{ auth: { token: token } });
        ...
        if (admin) {
            ...
            button.onclick = () => {
                ...
                form.addEventListener("submit", async (e) => {
                    ...
                    console.log("3333", newGroupName)
                    const newMembers = e.target.allMembers.value.split(",");
                    console.log("44444", newMembers)
...
document.addEventListener("DOMContentLoaded", async () => {
    try {
        if (!token) {
            ...
            const li = document.createElement("li");
            li.innerHTML = `<a class="active" href="/groups.html/${groupId}">${groupName}</a>`;
