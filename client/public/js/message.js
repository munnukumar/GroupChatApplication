...
async function fetchAllMessages() {
    ...
    try {
        const response = await axios.get("http://localhost:3000/all", { headers: { "Authorization": token } });
        const newMessages = response.data.message;
        if (!arraysEqual(previousMessages, newMessages)) {
            let content = '';
            const onlineUsersResponse = await axios.get("http://localhost:3000/online-users", { headers: { "Authorization": token } });
